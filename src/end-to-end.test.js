...
it("produces correct link for view before & after go is pressed", async () => {
  ...
  const expectedLinkBRCA1 =
    "http://localhost?name=snp1kg-BRCA1&tracks[0][files][0][type]=graph&tracks[0][files][0][name]=snp1kg-BRCA1.vg.xg&tracks[1][files][0][type]=read&tracks[1][files][0][name]=NA12878-BRCA1.sorted.gam&dataPath=default&region=17%3A1-100&bedFile=snp1kg-BRCA1.bed&dataType=built-in"
   // Set up dropdown
  await act(async () => {
    let dropdown = document.getElementById("dataSourceSelect");
    await userEvent.selectOptions(
  ...
  const expectedLinkCactus =
    "http://localhost?tracks[0][files][0][type]=graph&tracks[0][files][0][name]=cactus.vg.xg&tracks[1][files][0][type]=read&tracks[1][files][0][name]=cactus-NA12879.sorted.gam&bedFile=cactus.bed&name=cactus&region=ref%3A1-100&dataPath=mounted&dataType=built-in"
